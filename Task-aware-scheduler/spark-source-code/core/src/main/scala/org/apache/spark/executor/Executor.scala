...
private[spark] class Executor(
  ...
  private def submitExecutorData(): Unit = {
    logInfo("Submitting executor data to custom operator")

    val client: KubernetesClient = new DefaultKubernetesClient()

    val sparkExecutorContext = new CustomResourceDefinitionContext.Builder()
      .withName("sparkexecutors.example.com")
      .withGroup("example.com")
      .withScope("Namespaced")
      .withVersion("v1")
      .withPlural("sparkexecutors")
      .build()
    logInfo("SparkExecutorContext created")

    val executorName = s"executor-$executorId"
    logInfo(s"Creating resource with name: $executorName")

    val sparkExecutor = new GenericKubernetesResource()
    sparkExecutor.setApiVersion("example.com/v1")
    sparkExecutor.setKind("SparkExecutor")

    val metadata = new ObjectMeta()
    metadata.setName(executorName)
    sparkExecutor.setMetadata(metadata)
    logInfo(s"Metadata set with name: ${metadata.getName}")

    val spec = new java.util.HashMap[String, Any]()
    spec.put("executorId", executorId)
    spec.put("executorHostname", executorHostname)
    spec.put("osInfo", s"${System.getProperty("os.name")}, ${System.getProperty("os.version")}, ${System.getProperty("os.arch")}")
    spec.put("javaVersion", System.getProperty("java.version"))
    spec.put("numRunningTasks", 0)
    spec.put("type", "new executor")
    logInfo("Spec populated with executor details")

    sparkExecutor.setAdditionalProperty("spec", spec)
    logInfo("Spec added to sparkExecutor")

    client.genericKubernetesResources(sparkExecutorContext)
      .inNamespace("default")
      .resource(sparkExecutor)
      .create()
    logInfo("Custom resource created for executor")
  ...
  private def updateNumRunningTasks(executorId: String, increment: Boolean): Unit = {
    val client: KubernetesClient = new DefaultKubernetesClient()
    val sparkExecutorContext = new CustomResourceDefinitionContext.Builder()
      .withName("sparkexecutors.example.com")
      .withGroup("example.com")
    ...
    val executorName = s"executor-$executorId"
    val sparkExecutor = client.genericKubernetesResources(sparkExecutorContext)
      .inNamespace("default")
      .withName(executorName)
      .get()

    if (sparkExecutor != null) {
      val spec = sparkExecutor.get("spec").asInstanceOf[java.util.Map[String, Any]]
      val currentNumRunningTasks = spec.get("numRunningTasks").asInstanceOf[Int]
      val updatedNumRunningTasks = if (increment) currentNumRunningTasks + 1 else currentNumRunningTasks - 1
      spec.put("numRunningTasks", updatedNumRunningTasks)
      ...
      logError(s"Failed to find custom resource for executor $executorId")
    }
  ...
  class TaskRunner(
    ...
    val executorID = taskDescription.executorId
    ...
    override def run(): Unit = {
      ...
      try {
        ...
        val client: KubernetesClient = new DefaultKubernetesClient()
        logInfo("Kubernetes client created")

        val sparkExecMsgContext = new CustomResourceDefinitionContext.Builder()
          .withName("sparkexecmsg.example.com")
          .withGroup("example.com")
          .withScope("Namespaced")
          .withVersion("v1")
          .withPlural("sparkexecmsgs")
          .build()
        logInfo("SparkExecMsgContext created")

        val executorName = s"executor-$executorId-${UUID.randomUUID()}"

        val sparkExecMsg = new GenericKubernetesResource()
        sparkExecMsg.setApiVersion("example.com/v1")
        sparkExecMsg.setKind("SparkExecMsg")

        val metadata = new ObjectMeta()
        metadata.setName(executorName)
        sparkExecMsg.setMetadata(metadata)
        logInfo(s"Metadata set with name: ${metadata.getName}")

        val spec = new java.util.HashMap[String, Any]()
        spec.put("message", s"Finished $taskName")
        spec.put("executorId", executorId)
        spec.put("type", "message")
        spec.put("task_id", taskId.toString)

        sparkExecMsg.setAdditionalProperty("spec", spec)
        logInfo("Spec added to sparkExecMsg")

        client.genericKubernetesResources(sparkExecMsgContext)
          .inNamespace("default")
          .resource(sparkExecMsg)
          .create()
        logInfo("Custom resource created for executor")
