...
class CoarseGrainedSchedulerBackend(scheduler: TaskSchedulerImpl, val rpcEnv: RpcEnv)
  ...
  class DriverEndpoint extends IsolatedThreadSafeRpcEndpoint with Logging {
    ...
    val sparkTaskUpdateContext = new CustomResourceDefinitionContext.Builder()
      .withName("taskupdates.example.com")
      .withGroup("example.com")
      .withScope("Namespaced")
      .withVersion("v1")
      .withPlural("taskupdates")
      .build()
    logInfo("SparkTaskUpdateContext created")

    val sparkTaskUpdate = new GenericKubernetesResource()
    sparkTaskUpdate.setApiVersion("example.com/v1")
    sparkTaskUpdate.setKind("TaskUpdate")

    val crName = s"taskupdate-${UUID.randomUUID()}"

    val metadata = new ObjectMeta()
    metadata.setName(crName)
    sparkTaskUpdate.setMetadata(metadata)

    val spec = new java.util.HashMap[String, Any]()
    spec.put("completed", 0)
    spec.put("taskId", "")
    sparkTaskUpdate.setAdditionalProperty("spec", spec)
    logInfo("Spec added to sparkTaskUpdate")

    client.genericKubernetesResources(sparkTaskUpdateContext)
      .inNamespace("default")
      .resource(sparkTaskUpdate)
      .create()
    ...
    private def chooseExecutor(): String = {
      val client: KubernetesClient = new DefaultKubernetesClient()

      val sparkRequestContext = new CustomResourceDefinitionContext.Builder()
        .withName("sparkrequest.example.com")
        .withGroup("example.com")
        .withScope("Namespaced")
        .withVersion("v1")
        .withPlural("sparkrequests")
        .build()
      logInfo("SparkRequestContext created")

      val sparkRequest = new GenericKubernetesResource()
      sparkRequest.setApiVersion("example.com/v1")
      sparkRequest.setKind("SparkRequest")

      val metaName = s"request-${UUID.randomUUID()}"
      val metadata = new ObjectMeta()
      metadata.setName(metaName)
      sparkRequest.setMetadata(metadata)
      ...
      sparkRequest.setAdditionalProperty("spec", spec)
      logInfo("Spec added to sparkExecutor")

      client.genericKubernetesResources(sparkRequestContext)
        .inNamespace("default")
        .resource(sparkRequest)
        .create()
      logInfo("Custom resource created for executor")
    ...
    private def retrieveExecutor(metaName: String): String = {
      val client: KubernetesClient = new DefaultKubernetesClient()
      ...
      val watch = client.genericKubernetesResources(sparkRequestContext)
        .inNamespace("default")
        .withName(metaName)
        .watch(new Watcher[GenericKubernetesResource] {
          override def eventReceived(action: Watcher.Action, resource: GenericKubernetesResource): Unit = {
            val spec = resource.getAdditionalProperties.get("spec").asInstanceOf[java.util.Map[String, Any]]
            val id = spec.get("executorId").asInstanceOf[String]
            logInfo(s"ExecutorId retrieved before if - $id")
      ...
      // Delete the resource after retrieving the executorId
      client.genericKubernetesResources(sparkRequestContext)
        .inNamespace("default")
        .withName(metaName)
        .delete()
      logInfo("Custom resource deleted after retrieving executorId")

      client.close()
      logInfo(s"Retrieved executorId: $executorId")
      executorId
    }

    private def makeOffersOrig(): Unit = {
    ...
    private def makeOffers(): Unit = {
      ...
      val taskDescs = withLock {
        ...
        val chosenExecutorId = retrieveExecutor(objectName)

        val executorData = executorDataMap(chosenExecutorId)
        val workOffers = IndexedSeq(buildWorkerOffer(chosenExecutorId, executorData))
        scheduler.resourceOffers(workOffers, true)
      }
      if (taskDescs.nonEmpty) {
        launchTasks(taskDescs)
    ...
    private def launchTasks(tasks: Seq[Seq[TaskDescription]]): Unit = {
      ...
      // Iterate through each task and create a CR object for it
      for (task <- tasks.flatten) {
        // Serialize and encode the task
